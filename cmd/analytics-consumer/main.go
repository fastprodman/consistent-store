...
func main() {
	...
	shutdownTracing, err := observability.InitTracing(ctx, "analytics-consumer")
	if err != nil {
	...
	projector := analyticsout.NewProjector(provider)
	handler := analyticsservices.NewOrderCreatedHandler(projector)
	consumer := analyticsin.NewKafkaConsumer(broker, handler)
